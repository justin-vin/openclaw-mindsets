...
export async function createThread(forumId, title, firstMessage, logger, opts = {}) {
  const message = opts.embeds ? { embeds: opts.embeds } : { content: firstMessage };
  return api("POST", `/channels/${forumId}/threads`, { name: title, message }, logger);
}
